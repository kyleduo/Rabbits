...
8. Supporting multiple hosts with **allowed_host**.
9. **P** class providing convinient accesss for page link.
...
| Glossary  | Meanings                                 |
| :-------- | :--------------------------------------- |
| Rabbit    | Entry class of using Rabbits.            |
...
`asyncSetup`method will put the I/O operation into work thread so it would not block the ui thread. But you need handle the finish event to notify user when they can navigate through your app.
...
```json
{
  ...
  "mappings": {
    ...
    "demo://rabbits.kyleduo.com/test/listing": "LISTING",
    "demo://rabbits.kyleduo.com/test/{testing}": "TEST",
    "demo://rabbits.kyleduo.com/test_fragment": "TEST_F",
    "demo://rabbits.kyleduo.com/second/{id:l}": "SECOND",
    "demo://rabbits.kyleduo.com/common": "COMMON",
    "demo://rabbits.kyleduo.com/dump": "DUMP",
    "demo://rabbits.kyleduo.com/web": "WEB",
...
There are some conditions where you have not just one host for you service. This is more common when you have to load Waps in your WebView. Rabbits provides capability of unifying the navigation including native Activity/Fragment and urls in Waps. `allowed_hosts` just extends the scene.

`allowed_hosts` correspended to a list of hosts, which can be replacement of ones in mappings sections. For example, in the case above, **demo://rabbits.kyleduo.com** will be matched to **MAIN** page. And **demo://allowed.kyleduo.com** leads to same result since it was put in `allowed_hosts` list.

This mechanism will be helpful when you have different hosts for different service environments.

### P class

The **P** classs is generated by Rabbits automatically during build phase, which contains all constants in **mappings.json**. This is an example from mappings.json in last section:

```java
public final class P {
  public static final String MAIN = "demo://rabbits.kyleduo.com";
  public static final String TEST = "demo://rabbits.kyleduo.com/test";
  public static final String LISTING = "demo://rabbits.kyleduo.com/test/listing";
  public static final String TEST_F = "demo://rabbits.kyleduo.com/test_fragment";
  public static final String COMMON = "demo://rabbits.kyleduo.com/common";
  public static final String DUMP = "demo://rabbits.kyleduo.com/dump";
  public static final String WEB = "demo://rabbits.kyleduo.com/web";
  public static final String SECOND(long id) {
    return String.format("demo://rabbits.kyleduo.com/second/%d", id);
  }
  public static final String CRAZY(String name, int age, boolean finish) {
    return String.format("demo://rabbits.kyleduo.com/crazy/%s/%d/%b/end", name, age, finish);
  }
}
```

There are two expression form for url which are `constant` and `static methods`. Constant is an symbol for  common url, while methods for REST urls which has params in them. The name of constants and methods depends one the right side of each map in mappings.json.

Examples of the using of P classs would like these:

```java
// Traditional way.
Rabbit.from(this).to("/common").start();
// With P class.
Rabbit.from(this).to(P.COMMON).start();

// Traditional way.
String name = "kyleduo";
int age = 25;
boolean finish = false;
Rabbit.from(this).to("/crazy/" + name + "/" + age + "/" + finish + "/end").start();
// With P class.
Rabbit.from(this).to(P.CRAZY(name, age, finish)).start();
```

You will got **Constant-String-Free**, **Modify one place, Change everywhere** and **Code autocomplete** with P class.
